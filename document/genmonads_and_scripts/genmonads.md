...
- **Cross-file callees**: when a function body calls a name defined in a sibling `.c` file in the same directory (e.g. `list_append_raw.c` calls `list_tail` defined in `list_tail.c`), the generated `_rel_lib.v` emits `Require Import {callee}_rel_lib.` at the top and skips declaring a local `Parameter {callee}_M`. Callees without a sibling `.c` are skipped entirely. Because cross-file references depend on sibling libs, per-file Rocq checking can fail in isolation; run Rocq across the full set of generated `*_rel_lib.v` files (e.g. via the directory-mode invocation).
- **Cross-file callees in the synthesis context**: `collect_synthesis_context` lazily processes only the sibling `.c` files whose names appear as unresolved calls in the current file's bodies. Each sibling function becomes an `available_callee` entry tagged `cross_file=True` with `defined_in` and a full `externals.M` signature plus low-level `spec`. The prompt template labels callees as `(same-file)` vs `(cross-file)` and instructs the agent not to redeclare the `Parameter` for cross-file callees.
- **Data witnesses**: loop invariants of the form `exists w, ... t -> data == w ...` are desugared into typed `store(&(t->data), int, w)` predicates (struct field types come from `struct list { int data; ... };` declarations in the C source or locally-includable headers). `store(addr, T, var)` predicates are spliced verbatim into the translated annotation; when `T` is a scalar/boolean C type the third argument is promoted into the abstract loop state, otherwise (pointer types like `struct list *`) the predicate is preserved but `var` is not carried.
