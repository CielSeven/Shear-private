...
/*@ With {A} (storeA: Z -> A -> Assertion) (l: list(DL_Node A))(a: A)
    ...
    Ensure exists pt,
            dllseg_shift(storeA,list,pt,l) *
            dllseg_shift(storeA,pt,node,cons(Build_DL_Node(a, node), nil)) *
            store(field_addr(list,struct LOS_DL_LIST,pstPrev),struct LOS_DL_LIST *,node) *
            store(field_addr(node,struct LOS_DL_LIST,pstNext),struct LOS_DL_LIST *,list)
*/
{
