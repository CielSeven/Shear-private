...
/*@ highSpec
    ...
    Ensure storesortedLinkNode(storeA, &(targetSortList->sortLinkNode), mksortedLinkNode(a,t))
*/
;
unsigned long long OsSortLinkGetTargetExpireTime(unsigned long long currTime, const SortLinkList *targetSortList)
/*@ lSpec <= highSpec
    ...
    Ensure dllseg_shift_rev(storesortedLinkNode(storeA), &(targetSortList->sortLinkNode), x, l) && x->pstNext == &(targetSortList->sortLinkNode)
*/
;
...
/*@
  ...
  Ensure  exists (ts_post: tickState),
          SysTick != 0 && (&g_archTickTimer) != 0 &&
