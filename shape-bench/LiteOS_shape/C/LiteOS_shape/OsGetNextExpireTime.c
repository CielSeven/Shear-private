...
/*@ Import Coq Require Import SimpleC.EE.Applications_human.LiteOS_shape.lib.glob_vars_and_defs */
/*@ Import Coq Require Import SimpleC.EE.Applications_human.LiteOS_shape.lib.Los_Verify_State_def*/
...
/*@ Extern Coq (storesortedLinkNode: {A} -> (Z -> A -> Assertion) -> Z -> sortedLinkNode A -> Assertion)
               ...
               (sortedLinkNodeMappingList : {A} -> list (DL_Node(sortedLinkNode A)) -> list (DL_Node(sortedLinkNode A)))
...
/*@ Extern Coq (nil : {A} -> list A)
               ...
               (map : {A} {B} -> (A -> B) -> list A -> list B)
 */
/*@ include strategies "../los_sortlink_shape.strategies" */
/*@ Import Coq Require Import SimpleC.EE.Applications_human.LiteOS_shape.lib.sortlink */
/*@ Import Coq Require Import SimpleC.EE.Applications_human.LiteOS_shape.lib.dll*/
/*@ Import Coq Require Import SimpleC.EE.Applications_human.LiteOS_shape.lib.tick_backup*/
typedef enum {
...
/*@ highSpec
  ...
  Ensure  store_sorted_dll(storeA, &(sortHead->sortLink), l) * store(&(OS_SORT_LINK_UINT64_MAX), unsigned long long, (2^64)-1) 
*/
;
static inline unsigned long long GetSortLinkNextExpireTime(SortLinkAttribute *sortHead, unsigned long long startTime, unsigned long long tickPrecision)
/*@ taskSpec <= highSpec
  ...
  Ensure   store_task_sorted_dll(sg, l) * store(&(OS_SORT_LINK_UINT64_MAX), unsigned long long, (2^64)-1)
*/
;
static inline unsigned long long GetSortLinkNextExpireTime(SortLinkAttribute *sortHead, unsigned long long startTime, unsigned long long tickPrecision)
/*@ swmtrSpec <= highSpec
  ...
  Ensure   store_swtmr_sorted_dll(sg, l) * store(&(OS_SORT_LINK_UINT64_MAX), unsigned long long, (2^64)-1)
*/
;
...
/*@
  ...
  Ensure   store_task_sorted_dll(sg, l1) * store_swtmr_sorted_dll(sg, l2) * store(&(OS_SORT_LINK_UINT64_MAX), unsigned long long, (2^64)-1)
*/
{
