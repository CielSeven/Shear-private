...
Definition increasing_aux {A: Type} (_a: DL_Node(sortedLinkNode A)) (_l: list (DL_Node(sortedLinkNode A))): Prop :=
    True.
...
Definition sortedLinkNodeMappingList {A: Type} (l : list (DL_Node(sortedLinkNode A))) : list (DL_Node(sortedLinkNode A)) :=
...
Lemma store_sorted_dll_split_shape:
    forall(A: Type) (x: Z)(t: Z) (storeA: Z -> A -> Assertion) (a: A) (l1 l2: list (DL_Node(sortedLinkNode A))) (sortList: Z),
    store_sorted_dll storeA x (l1 ++ (Build_DL_Node (mksortedLinkNode a t) sortList) :: l2) |--
    EX h pt py z,
    storeA (&(sortList # "SortLinkList" ->ₛ "sortLinkNode")) a **
    (&( sortList # "SortLinkList" ->ₛ "responseTime") # UInt64 |-> t **
    (&( ((&( ((sortList)) # "SortLinkList" ->ₛ "sortLinkNode"))) # "LOS_DL_LIST" ->ₛ "pstPrev") # Ptr |-> py **
    (&( ((&( ((sortList)) # "SortLinkList" ->ₛ "sortLinkNode"))) # "LOS_DL_LIST" ->ₛ "pstNext") # Ptr |-> z **
    (dllseg (storesortedLinkNode storeA) z (&(sortList # "SortLinkList" ->ₛ "sortLinkNode")) x pt (map sortedLinkNodeMapping l2) **
        (&( x # "LOS_DL_LIST" ->ₛ "pstPrev") # Ptr |-> pt **
        (&( x # "LOS_DL_LIST" ->ₛ "pstNext") # Ptr |-> h **
        dllseg (storesortedLinkNode storeA) h x (&(sortList # "SortLinkList" ->ₛ "sortLinkNode")) py (map sortedLinkNodeMapping l1))))))).
Proof.
    unfold store_sorted_dll.
    intros.
    Intros.
...
Lemma store_sorted_dll_concat_shape:
forall(A: Type) (x : Z) (h pt py z: Z)(t: Z) (storeA: Z -> A -> Assertion) (a: A)
      (l1 l2: list (DL_Node(sortedLinkNode A))) (sortList: Z),
    storeA &( sortList # "SortLinkList" ->ₛ "sortLinkNode") a **
    (&( sortList # "SortLinkList" ->ₛ "responseTime") # UInt64 |-> t **
    (&( ((&( ((sortList)) # "SortLinkList" ->ₛ "sortLinkNode"))) # "LOS_DL_LIST" ->ₛ "pstPrev") # Ptr |-> py **
    (&( ((&( ((sortList)) # "SortLinkList" ->ₛ "sortLinkNode"))) # "LOS_DL_LIST" ->ₛ "pstNext") # Ptr |-> z **
    (dllseg (storesortedLinkNode storeA) z &( sortList # "SortLinkList" ->ₛ "sortLinkNode") x pt (sortedLinkNodeMappingList l2) **
        (&( x # "LOS_DL_LIST" ->ₛ "pstPrev") # Ptr |-> pt **
        (&( x # "LOS_DL_LIST" ->ₛ "pstNext") # Ptr |-> h **
        dllseg (storesortedLinkNode storeA) h x &( sortList # "SortLinkList" ->ₛ "sortLinkNode") py (sortedLinkNodeMappingList l1))))))) |--
    store_sorted_dll storeA x (l1 ++ (Build_DL_Node (mksortedLinkNode a t) sortList) :: l2).
Proof.
    intros.
    ...
    - entailer!.
    - apply (store_sorted_dll_concat A x h pt (&(sortList # "SortLinkList" ->ₛ "sortLinkNode")) py z t storeA a l1 l2 sortList).
      + reflexivity.
      + unfold increasing; exact I.
Qed.
