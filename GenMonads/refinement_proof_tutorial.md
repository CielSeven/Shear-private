...
2. `rewrite (repeat_break_unfold _ _) in`: rewrites `repeat_break` using its unfolding lemma inside the specified hypothesis or goal.

    After `rewrite (repeat_break_unfold _ _) in H4.`:
    ```
    1 goal
